...
if casting_number and scale_number:
    base_id = f"{casting_number}-{scale_number}"
...
if "transactions" not in st.session_state:
    st.session_state.transactions = pd.DataFrame(columns=["Transaction ID", "Weight (kg)", "Date & Time", "Bundle Size"])

# Add Transaction
st.write("### Add Transaction")
weight = st.number_input("Enter Weight for Transaction (kg)", min_value=0.0, step=0.1)
if st.button("Add Transaction"):
    # Generate transaction ID
    transaction_count = len(st.session_state.transactions) + 1
    transaction_id = f"{base_id}-{str(transaction_count).zfill(3)}"
    ...
    st.success(f"Transaction {transaction_id} added successfully with weight {weight} kg.")
...
if not st.session_state.transactions.empty:
    ...
    # QR code data to include Company, Country of Origin, Transaction ID, Date & Time, Weight, and Bundle Size
    qr_data = (
        f"Country of Origin: DR Congo\n"
        f"Company Name: Smelter\n"
        f"Transaction ID: {latest_transaction['Transaction ID']}\n"
        f"Date & Time: {latest_transaction['Date & Time']}\n"
        f"Weight: {latest_transaction['Weight (kg)']} kg\n"
    ...
    st.write(f"**Country of Origin:** DR Congo")
    st.write(f"**Company Name:** Smelter")
    st.write(f"**Transaction ID:** {latest_transaction['Transaction ID']}")
    st.write(f"**Date & Time:** {latest_transaction['Date & Time']}")
    st.write(f"**Weight:** {latest_transaction['Weight (kg)']} kg")
